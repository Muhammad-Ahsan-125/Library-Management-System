...
void Library_System::reserveResource(User &u)
{
    ...
    // STEP 1: User cannot reserve a resource already borrowed by himself
    for (auto &b : borrows)
    {
    ...
    // STEP 2: If copies are available, user should borrow directly
    ...
    // STEP 3: User cannot reserve the same resource twice
...
void Library_System::addResource()
{
    ...
    if (choice == 1)
    {
        r = new Book();     // Dynmaic memory allocation
        r->input();         // Runtime polymorphism
    }
    else if (choice == 2)
    {
        r = new Magazine();
    ...
    if (r != nullptr)
    {
        // ===== Checking duplication of ISBN or ISSN =====
...
void Library_System::viewAllResources()
{
    cout << "\n---- All Resources ----\n";
    for (auto r : resources)
        r->display(); // Runtime Polymorphism
}
...
void Library_System::borrowResource(User &u)
{
    ...
    // STEP 1: Same user cannot borrow the same resource again
    for (auto &b : borrows)
    {
        if (b.username == u.getUsername() &&
            b.resourceIdentifier == selected->getIdentifier() &&
    ...
    // STEP 2: Reservation priority check
    int reservedIndex = -1;
    ...
    // STEP 4: If current user had reserved this resource, remove reservation
    if (reservedIndex != -1 &&
        reservations[reservedIndex].username == u.getUsername())
    {
...
void Library_System::deleteResource()
{
    ...
    for (int i = 0; i < (int)resources.size(); i++)
    {
        ...
        {
            //Deleting all the reservations of this resource
            reservations.erase(
